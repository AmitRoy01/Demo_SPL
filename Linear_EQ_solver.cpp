...
double find_determinant(double arr[][SIZE], unsigned int number)
{
...
bool check_if_infinite_solution(double result[], int number_of_variable)
{
...
void solution_process(int number_of_variable)
{
    ...
    result[0] = find_determinant(matrix,number);             // make determine for constant

    for(size_t i=0; i<number; i++)
    {
        replace_column(i,number);
        result[i+1] = find_determinant(temp,number);         //find determine for each variable
    }
    ...
    {
        if(check_if_infinite_solution(result, number_of_variable))
        {
...
int take_input(string str[])
{
    ...
    cout<<"Input "<<number <<"equation one by one "<<endl;
    for(size_t i=0; i<number; i++)
    {
...
{
    int num = extract_variable_number(str, number);
